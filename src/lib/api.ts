...
export const documentApi = {
  ...
  getDisplayType(backendType: DocumentType): DisplayDocumentType {
    const entry = Object.entries(DOCUMENT_TYPE_MAP).find(
      ([_, value]) => value === backendType,
    );
  ...
  calculateProgress(documents: BackendDocument[]): number {
    ...
    const percentage = Math.min(
      100,
      Math.round((uploadedTypes / totalTypes) * 100),
    );
    return percentage;
  },

  // Helper to check if a document type is uploaded
  isDocumentTypeUploaded(
...
export const profileApi = {
  ...
  async getProgramsByLevel(
    ...
    const response = await fetch(
      `${API_BASE_URL}/profile/programs?currentLevel=${currentLevel}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
...
export const universityApi = {
  ...
  async getUniversities(filters?: {
    ...
    if (filters) {
      ...
      if (filters.program_level)
        params.append("program_level", filters.program_level);
    ...
    const universities: UniversityListResponse[] = await response.json();

    // Node endpoint currently supports country/program_level server-side.
    // Keep min_gpa/max_tuition filtering client-side for UI compatibility.
    return universities.filter((university) => {
      if (filters?.min_gpa && university.min_gpa > filters.min_gpa) {
        return false;
      }

      if (
        filters?.max_tuition &&
        university.tuition_fee_usd > filters.max_tuition
      ) {
        return false;
      }
  ...
  convertToFrontendFormat(
    university:
      ...
      | UniversityRecommendationResponse["university"],
    ...
    const safeUniversity = {
      ...
      avg_scholarship_percentage:
        (university as any).avg_scholarship_percentage || null,
      fields_offered: (university as any).fields_offered || [
        university.program_name,
      ],
      requires_portfolio: (university as any).requires_portfolio || false,
      requires_research_proposal:
        (university as any).requires_research_proposal || false,
      ...
      employment_rate_6_months:
        (university as any).employment_rate_6_months || null,
      avg_starting_salary_usd:
        (university as any).avg_starting_salary_usd || null,
