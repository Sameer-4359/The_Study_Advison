...
export default function StepCard({ title, subtitle, duration, progressPercent = 0, status = "pending", actionLabel }: Props) {
  const bgByStatus = {
  ...
  return (
    <div className={`rounded-xl border ${bgByStatus} p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4`}>
      <div className="flex-1">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="hidden sm:block">
            <button className={`px-3 py-1 rounded-md text-xs font-semibold ${actionStyle}`}>{actionLabel || "Start"}</button>
          </div>
        </div>
      ...
      <div className="sm:hidden w-full">
        <button className={`w-full mt-2 py-2 rounded-md text-sm font-semibold ${actionStyle}`}>{actionLabel || "Start"}</button>
      </div>
    </div>
  );
}
